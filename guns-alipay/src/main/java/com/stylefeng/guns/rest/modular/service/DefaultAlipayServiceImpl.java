...
public class DefaultAlipayServiceImpl implements AliPayServiceAPI {
    ...
    public String trade_precreate(String orderId) {
        ...
        String operatorId = "Duoclassic";

        // (必填) 商户门店编号，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        String storeId = "Duoclassic";
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                filePath = String.format("F:/tmp/qrCode/qr-%s.png",
                        response.getOutTradeNo());
                String fileName=String.format("qr-%s.png",response.getOutTradeNo());
                log.info("filePath:" + filePath);
                File qrCodeImage = ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                boolean isSuccess = ftpUtil.uploadFle(fileName, qrCodeImage);
                if (!isSuccess){
                    filePath="";
                    log.error("二维码上传失败");
                }
